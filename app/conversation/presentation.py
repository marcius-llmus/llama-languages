...
from app.conversation.schemas import WebSocketMessage
        ...
        try:
            while True:
                ...
                if message.text_message:
                    ...
                    user_message_data = message.text_message
                elif message.audio_message:
                    logger.info("Received audio message from client.")
                    header, encoded = message.audio_message.split(",", 1)
                    user_message_data = base64.b64decode(encoded)
                else:
                    # This case should be prevented by the model validator
                    logger.warning("Received valid WebSocket message but no content.")
                    continue
                ...
                stream = self.conversation_service.run_conversation_turn(
                    ...
                    practice_topic_id=message.practice_topic_id,
                )
